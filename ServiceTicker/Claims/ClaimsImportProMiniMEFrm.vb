...
Public Class ClaimsImportProMiniMEFrm
    Dim LinearEncoder As New BarcodeLib.Barcode

    Private Sub ClaimsImportProMiniMEFrm_FormClosing(sender As Object, e As FormClosingEventArgs) Handles Me.FormClosing
        Me.Dispose()
        OnTopFrm()
    End Sub


    Private Sub ClaimsImportProMiniMEFrm_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    Private Sub Button1_Click(sender As Object, e As EventArgs) Handles Button1.Click
        If Me.rbCIPreturnStockMe.Checked = True Then
            Try
                ...
                With cmd
                    .Parameters.AddWithValue("@code_pro", Me.dgCIPme.Rows(0).Cells(7).Value)
                    .Parameters.AddWithValue("@serial_pro", Me.dgCIPme.Rows(0).Cells(5).Value)
                    .Parameters.AddWithValue("@name_pro", Me.dgCIPme.Rows(0).Cells(8).Value)
            ...
            Dim scp As Integer
            Try
                strSQL = "SELECT count_num FROM product WHERE code_pro='" & Me.dgCIPme.Rows(0).Cells(7).Value & "'"
                cmd = New MySqlCommand(strSQL, ConnectionDB)
                Call open_connection()
                Dr = cmd.ExecuteReader()
                Dr.Read()
                cp = Dr.GetInt64("count_num")
                scp = cp + 1
            Catch ex As Exception
                MainFrm.lblMainStatus.Text = "Error - " & Me.GetType().Name & " - " & System.Reflection.MethodBase.GetCurrentMethod().Name & " - " & ex.Message
            End Try
            ...
            Try
                ...
                strSQL = "UPDATE product SET count_num='" & scp & "' WHERE code_pro='" & Me.dgCIPme.Rows(0).Cells(7).Value & "'"
                cmd = New MySqlCommand(strSQL, ConnectionDB)
                Call open_connection()
                cmd.ExecuteNonQuery()
            Catch ex As Exception
            ...
            Try
                ...
                strSQL = "UPDATE claims SET `status`='ส่งคืนแล้ว' WHERE claims_id='" & Me.dgCIPme.Rows(0).Cells(1).Value & "'"
                cmd = New MySqlCommand(strSQL, ConnectionDB)
                Call open_connection()
                cmd.ExecuteNonQuery()
            Catch ex As Exception
            ...
            Try
                'update claims send
                strSQL = "UPDATE claims_send SET claims_send_status='ส่งคืนแล้ว' WHERE claims_id='" & Me.dgCIPme.Rows(0).Cells(1).Value & "'"
                cmd = New MySqlCommand(strSQL, ConnectionDB)
                Call open_connection()
                cmd.ExecuteNonQuery()
            Catch ex As Exception
            ...
            Try
                ...
                strSQL = "INSERT INTO claims_copy(claims_id,customer_id,serial_pro,code_pro,bar_code,name_pro,price_buy,price_wholesale,price_technician,price_loyel,price_members,`type`,unit,datetime_sale,datetime_return,datetime_edit,claims_money,employee_claims_return,tel_to_cus,employee_sale,symptom,accressory,note,note2,`status`,employee,datetime_save,claims_id_pic)" _
                    & "SELECT * FROM claims WHERE claims_id='" & Me.dgCIPme.Rows(0).Cells(1).Value & "'"
                cmd = New MySqlCommand(strSQL, ConnectionDB)
                Call open_connection()
                cmd.ExecuteNonQuery()
            Catch ex As Exception
            ...
            Try
                strSQL = "DELETE FROM claims WHERE claims_id='" & Me.dgCIPme.Rows(0).Cells(1).Value & "'"
                cmd = New MySqlCommand(strSQL, ConnectionDB)
                Call open_connection()
                cmd.ExecuteNonQuery()
            Catch ex As Exception
        ...
        ElseIf Me.rbCIPreturnClaimsMe.Checked = True Then
            ...
            Try
                strSQL = "SELECT code_pro,bar_code,name_pro," _
                    ...
                    & "unit,datetime_sale,employee_sale FROM claims WHERE claims_id='" & Me.dgCIPme.Rows(0).Cells(1).Value & "'"
                cmd = New MySqlCommand(strSQL, ConnectionDB)
                Call open_connection()
                Dr = cmd.ExecuteReader()
                Dr.Read()
            ...
            Try
                ...
                With cmd
                    ...
                    .Parameters.Add(New MySqlParameter("@customer_id", Me.dgCIPme.Rows(0).Cells(2).Value))
                    .Parameters.Add(New MySqlParameter("@serial_pro", Me.dgCIPme.Rows(0).Cells(5).Value))
                    .Parameters.Add(New MySqlParameter("@code_pro", Me.dgCIPme.Rows(0).Cells(7).Value))
                    ...
                    .Parameters.Add(New MySqlParameter("@symptom", Me.dgCIPme.Rows(0).Cells(9).Value))
                    .Parameters.Add(New MySqlParameter("@accressory", Me.dgCIPme.Rows(0).Cells(10).Value))
            ...
            Try
                ...
                strSQL = "UPDATE claims SET " _
                     ...
                     & "`status`='ส่งคืนแล้ว' WHERE claims_id='" & Me.dgCIPme.Rows(0).Cells(1).Value & "'"
                cmd = New MySqlCommand(strSQL, ConnectionDB)
                Call open_connection()
                cmd.ExecuteNonQuery()
            Catch ex As Exception
            ...
            Try
                ...
                strSQL = "UPDATE claims_send SET claims_send_status='ส่งคืนแล้ว'," _
                    & "datetime_edit='" & DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss",
                                   CultureInfo.CreateSpecificCulture("en-US")) & "' WHERE claims_id='" & Me.dgCIPme.Rows(0).Cells(1).Value & "'"
                cmd = New MySqlCommand(strSQL, ConnectionDB)
                Call open_connection()
                cmd.ExecuteNonQuery()
            Catch ex As Exception
