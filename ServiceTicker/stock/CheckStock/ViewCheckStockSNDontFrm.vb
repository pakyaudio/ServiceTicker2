...
Public Class ViewCheckStockSNDontFrm
    ...
    Private Sub GetSNByCodepro(ByVal cp)
        ...
        Try
            If cp = "all" Then
                cmd = New MySqlCommand("SELECT serial_pro,name_pro FROM check_stock LIMIT " & Me.NumericUpDown1.Value & ";", ConnectionDB)
            Else
                cmd = New MySqlCommand("SELECT serial_pro,name_pro FROM check_stock " _
        ...
        Try
            If cp = "all" Then
                cmd = New MySqlCommand("SELECT COUNT(code_pro) AS cid FROM check_stock;", ConnectionDB)
            Else
                cmd = New MySqlCommand("SELECT COUNT(code_pro) AS cid FROM check_stock WHERE code_pro='" & cp & "';", ConnectionDB)
            End If
            Call open_connection()
            Dr = cmd.ExecuteReader
            If Dr.HasRows Then
    ...
    Private Sub codeproSelect2Next(ByVal cp)
        Try
            ...
            Else
                cmd = New MySqlCommand("SELECT serial_pro,name_pro FROM " _
                                     & "check_stock WHERE code_pro='" & cp & "' " _
                                     & "LIMIT " & Me.NumericUpDown1.Value & " OFFSET " & Me.DataGridView1.Rows.Count & ";", ConnectionDB)
            End If
    ...
    Private Sub Button1_Click(sender As Object, e As EventArgs) Handles Button1.Click
        If Me.DataGridView1.Rows.Count <> 0 Then
            If MessageBox.Show(Me, "ต้องการพิมพ์รายการนี้", "ยืนยัน", MessageBoxButtons.OKCancel, MessageBoxIcon.Question) = Windows.Forms.DialogResult.OK Then
                Try
                    strSQLite = "DELETE FROM check_stock_dont;"
                ...
                For r As Integer = 0 To Me.DataGridView1.Rows.Count - 1
                    Try
                        strSQLite = "INSERT INTO check_stock_dont(no,serial_pro,name_pro)VALUES(@no,@serial_pro,@name_pro)"
                        cmdSQLite = New SQLite.SQLiteCommand(strSQLite, connSQLite)
                ...
                If checkPrinter() = "A4" Then
                    CheckStockDontCheckA4ReportPrintFrm.ShowDialog(Me)
                ElseIf checkPrinter() = "A5" Then
                    CheckStockDontCheckA5ReportPrintFrm.ShowDialog(Me)
                End If
            End If
        End If
    End Sub
