...
Public Class FixcomFrm
    ...
    Private Sub getData()
        Try
            ...
            cmd = New MySqlCommand("SELECT `ID`,fix_name FROM fix_com ORDER BY fix_name;", ConnectionDB)
            ...
            While Dr.Read()
                With Me.dgProductType
                    ...
                    .Rows(r).Cells(0).Value = Dr.GetString("fix_name")
                End With
            End While
    ...
    Private Sub FixcomFrm_FormClosing(sender As Object, e As FormClosingEventArgs) Handles Me.FormClosing
        Me.Dispose()
        OnTopFrm()
    End Sub
    ...
    Private Sub InsertData()
        If checkMySQLDruplicateDB("SELECT fix_name FROM fix_com WHERE fix_name='" & Me.txtEdit.Text & "'") = True Then
            MessageBox.Show(Me, "มีรายการนี้อยู่แล้ว กรุณาเปลี่ยนเป็นอย่างอื่น", "ข้อมูลซ้ำ", MessageBoxButtons.OK, MessageBoxIcon.Stop)
        Else
            Try
                cmd = New MySqlCommand("INSERT INTO fix_com(fix_name)VALUES('" & Me.txtEdit.Text & "')", ConnectionDB)
                Call open_connection()
                Dim save As Integer
    ...
    Private Sub UpdateData()
        If checkMySQLDruplicateDB("SELECT fix_name FROM fix_com WHERE " _
                                & "fix_name='" & Me.txtEdit.Text & "' AND NOT fix_name='" & Me.dgProductType.CurrentRow.Cells(0).Value & "'") = True Then
            MessageBox.Show(Me, "มีรายการนี้อยู่แล้ว กรุณาเปลี่ยนเป็นอย่างอื่น", "ข้อมูลซ้ำ", MessageBoxButtons.OK, MessageBoxIcon.Stop)
        Else
            Try
                cmd = New MySqlCommand("UPDATE fix_com SET fix_name='" & Me.txtEdit.Text & "' WHERE ID='" & Me.dgProductType.CurrentRow.Cells(0).Tag & "'", ConnectionDB)
                Call open_connection()
                Dim save As Integer
                save = cmd.ExecuteNonQuery()
                If save = 1 Then
    ...
    Private Sub DeleteData()
        Try
            cmd = New MySqlCommand("DELETE FROM fix_com WHERE fix_name='" & Me.txtEdit.Text & "'", ConnectionDB)
            Call open_connection()
            Dim save As Integer
            save = cmd.ExecuteNonQuery()
            If save = 1 Then
    ...
    Private Sub btnHelp_Click(sender As Object, e As EventArgs) Handles btnHelp.Click
        Try
            Process.Start("https://sites.google.com/site/pakyaudio/home/serviceticker/comfix/comfixconfig/fixcomfrm")
        Catch ex As Exception
