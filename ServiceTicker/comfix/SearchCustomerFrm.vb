...
Public Class SearchCustomerFrm
    ...
    Private Sub btnSendtoSavedatafix_Click(sender As Object, e As EventArgs) Handles btnSendtoSavedatafix.Click
        If Me.dgSearchCustoSaveFix.Rows.Count <> 0 Then
            If Me.dgSearchCustoSaveFix.CurrentRow.Cells(0).Value = "0" Then
                MessageBox.Show(Me, "ใช้ชื่อลูกค้าเงินสดในงานรับซ่อมไม่ได้", "แจ้งเตือน", MessageBoxButtons.OK, MessageBoxIcon.Stop)
            Else
                If Me.btnSendtoSavedatafix.Text = "เลือกไปหน้ารับงานซ่อม" Then
                    SaveDataFixFrm.txtSDFCusID.Text = Me.dgSearchCustoSaveFix.CurrentRow.Cells(0).Value
                    SaveDataFixFrm.txtSDFcusname.Text = Me.dgSearchCustoSaveFix.CurrentRow.Cells(1).Value
                    SaveDataFixFrm.txtSDFcusaddress.Text = Me.dgSearchCustoSaveFix.CurrentRow.Cells(2).Value
                    SaveDataFixFrm.txtSDFcuszipcode.Text = Me.dgSearchCustoSaveFix.CurrentRow.Cells(3).Value
                    SaveDataFixFrm.txtSDFcusTel.Text = Me.dgSearchCustoSaveFix.CurrentRow.Cells(4).Value
                    Me.Close()
                ElseIf Me.btnSendtoSavedatafix.Text = "เลือกไปหน้าเคลมสินค้า" Then
                    ClaimsFrm.txtCFcustomer_id.Text = Me.dgSearchCustoSaveFix.CurrentRow.Cells(0).Value
                    ClaimsFrm.txtCFcustomer_name.Text = Me.dgSearchCustoSaveFix.CurrentRow.Cells(1).Value
                    Me.Close()
                End If
            End If
        End If
