...
Public Class AccessoryEditFrm
    ...
    Private Sub getData()
        Try
            ...
            cmd = New MySqlCommand("SELECT ID,accessory_name FROM accessory ORDER BY accessory_name", ConnectionDB)
            ...
            While Dr.Read()
                With Me.dgProductType
                    ...
                    .Rows(r).Cells(0).Value = Dr.Item("accessory_name")
                End With
            End While
    ...
    Private Sub AccessoryEditFrm_FormClosing(sender As Object, e As FormClosingEventArgs) Handles Me.FormClosing
        Me.Dispose()
        OnTopFrm()
    End Sub
    ...
    Private Sub InsertData()
        If checkMySQLDruplicateDB("SELECT accessory_name FROM accessory WHERE accessory_name='" & Me.txtEdit.Text & "'") = True Then
            MessageBox.Show(Me, "มีรายการนี้อยู่แล้ว กรุณาเปลี่ยนเป็นอย่างอื่น", "ข้อมูลซ้ำ", MessageBoxButtons.OK, MessageBoxIcon.Stop)
        Else
            Try
                cmd = New MySqlCommand("INSERT INTO accessory(accessory_name)VALUES('" & Me.txtEdit.Text & "')", ConnectionDB)
                Call open_connection()
                Dim save As Integer
    ...
    Private Sub UpdateData()
        If checkMySQLDruplicateDB("SELECT accessory_name FROM accessory WHERE " _
                                & "accessory_name='" & Me.txtEdit.Text & "' AND NOT accessory_name='" & Me.dgProductType.CurrentRow.Cells(0).Value & "'") = True Then
            MessageBox.Show(Me, "มีรายการนี้อยู่แล้ว กรุณาเปลี่ยนเป็นอย่างอื่น", "ข้อมูลซ้ำ", MessageBoxButtons.OK, MessageBoxIcon.Stop)
        Else
            Try
                cmd = New MySqlCommand("UPDATE accessory SET accessory_name='" & Me.txtEdit.Text & "' WHERE ID='" & Me.dgProductType.CurrentRow.Cells(0).Tag & "'", ConnectionDB)
                Call open_connection()
                Dim save As Integer
                save = cmd.ExecuteNonQuery()
                If save = 1 Then
    ...
    Private Sub DeleteData()
        Try
            cmd = New MySqlCommand("DELETE FROM accessory WHERE ID='" & Me.dgProductType.CurrentRow.Cells(0).Tag & "'", ConnectionDB)
            Call open_connection()
            Dim save As Integer
            save = cmd.ExecuteNonQuery()
            If save = 1 Then
    ...
    Private Sub btnHelp_Click(sender As Object, e As EventArgs) Handles btnHelp.Click
        Try
            Process.Start("https://sites.google.com/site/pakyaudio/home/serviceticker/comfix/comfixconfig/accessoryeditfrm")
        Catch ex As Exception
