...
Public Class SearchCustomerCCFrm
    ...
    Private Sub dgSearchCustoSaveFix_CellDoubleClick(sender As Object, e As DataGridViewCellEventArgs) Handles dgSearchCustoSaveFix.CellDoubleClick
        If Me.dgSearchCustoSaveFix.Rows.Count <> 0 Then
            If Me.btnSendtoSavedatafix.Text = "เลือกไปหน้าสถิติงานช่าง" Then
                ComfixCustomerStatusFrm.bsearch = 100
                ComfixCustomerStatusFrm.getCustomerStatus(Me.dgSearchCustoSaveFix.SelectedCells(0).Value)
                MessageBox.Show(Me, "เพิ่มข้อมูลแล้ว", "แจ้งเตือน", MessageBoxButtons.OK, MessageBoxIcon.Information)
            End If
            'Me.Close()
        End If
    End Sub
    ...
    Private Sub SearchCustomerCCFrm_FormClosing(sender As Object, e As FormClosingEventArgs) Handles Me.FormClosing
        Me.Dispose()
        OnTopFrm()
    End Sub
