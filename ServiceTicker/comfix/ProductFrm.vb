...
Public Class ProductFrm
    ...
    Private Sub getData()
        Try
            ...
            cmd = New MySqlCommand("SELECT ID,p_type FROM product_type ORDER BY P_type", ConnectionDB)
            ...
            While Dr.Read()
                With Me.dgProductType
                    ...
                    .Rows(r).Cells(0).Value = Dr.Item("p_type")
                End With
            End While
    ...
    Private Sub ProductFrm_FormClosing(sender As Object, e As FormClosingEventArgs) Handles Me.FormClosing
        Me.Dispose()
        OnTopFrm()
    End Sub
    ...
    Private Sub dgProductType_CellClick(sender As Object, e As DataGridViewCellEventArgs) Handles dgProductType.CellClick
        If Me.dgProductType.Rows.Count <> 0 Then
            ...
            Me.txtEdit.Enabled = False
            Me.txtEdit.Text = Me.dgProductType.CurrentRow.Cells(1).Value
        End If
    End Sub
    ...
    Private Sub InsertData()
        If checkMySQLDruplicateDB("SELECT p_type FROM product_type WHERE p_type='" & Me.txtEdit.Text & "'") = True Then
            MessageBox.Show(Me, "มีรายการนี้อยู่แล้ว กรุณาเปลี่ยนเป็นอย่างอื่น", "ข้อมูลซ้ำ", MessageBoxButtons.OK, MessageBoxIcon.Stop)
        Else
            Try
                cmd = New MySqlCommand("INSERT INTO product_type(p_type)VALUES('" & Me.txtEdit.Text & "')", ConnectionDB)
                Call open_connection()
                Dim save As Integer
    ...
    Private Sub UpdateData()
        If checkMySQLDruplicateDB("SELECT p_type FROM product_type WHERE " _
                                  & "p_type='" & Me.txtEdit.Text & "' AND NOT p_type='" & Me.dgProductType.CurrentRow.Cells(0).Value & "'") = True Then
            MessageBox.Show(Me, "มีรายการนี้อยู่แล้ว กรุณาเปลี่ยนเป็นอย่างอื่น", "ข้อมูลซ้ำ", MessageBoxButtons.OK, MessageBoxIcon.Stop)
        Else
            Try
                cmd = New MySqlCommand("UPDATE product_type SET p_type='" & Me.txtEdit.Text & "' WHERE ID='" & Me.dgProductType.CurrentRow.Cells(0).Tag & "'", ConnectionDB)
                Call open_connection()
                Dim save As Integer
                save = cmd.ExecuteNonQuery()
                If save = 1 Then
    ...
    Private Sub DeleteData()
        Try
            cmd = New MySqlCommand("DELETE FROM product_type WHERE p_type='" & Me.txtEdit.Text & "'", ConnectionDB)
            Call open_connection()
            Dim save As Integer
            save = cmd.ExecuteNonQuery()
            If save = 1 Then
    ...
    Private Sub btnHelp_Click(sender As Object, e As EventArgs) Handles btnHelp.Click
        Try
            Process.Start("https://sites.google.com/site/pakyaudio/home/serviceticker/comfix/comfixconfig/productfrm")
        Catch ex As Exception
