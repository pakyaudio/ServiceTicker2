...
Public Class ProductNameFrm
        ...
        Try
            ...
            While Dr.Read()
                With Me.dgProductType
                    r = .RowCount
                    .Rows.Add()
                    .Rows(r).Cells(0).Tag = Dr.GetString("ID")
                    .Rows(r).Cells(0).Value = Dr.GetString("p_name")
                End With
            End While
    ...
    Private Sub ProductNameFrm_FormClosing(sender As Object, e As FormClosingEventArgs) Handles Me.FormClosing
        Me.Dispose()
        OnTopFrm()
    End Sub
    ...
    Private Sub InsertData()
        If checkMySQLDruplicateDB("SELECT p_name FROM product_name WHERE p_name='" & Me.txtEdit.Text & "'") = True Then
            MessageBox.Show(Me, "มีรายการนี้อยู่แล้ว กรุณาเปลี่ยนเป็นอย่างอื่น", "ข้อมูลซ้ำ", MessageBoxButtons.OK, MessageBoxIcon.Stop)
        Else
            Try
                cmd = New MySqlCommand("INSERT INTO product_name(p_name)VALUES('" & Me.txtEdit.Text & "')", ConnectionDB)
                Call open_connection()
                Dim save As Integer
    ...
    Private Sub UpdateData()
        If checkMySQLDruplicateDB("SELECT p_name FROM product_name WHERE " _
                                & "p_name='" & Me.txtEdit.Text & "' AND NOT p_name='" & Me.dgProductType.CurrentRow.Cells(0).Value & "'") = True Then
            MessageBox.Show(Me, "มีรายการนี้อยู่แล้ว กรุณาเปลี่ยนเป็นอย่างอื่น", "ข้อมูลซ้ำ", MessageBoxButtons.OK, MessageBoxIcon.Stop)
        Else
            Try
                cmd = New MySqlCommand("UPDATE product_name SET p_name='" & Me.txtEdit.Text & "' WHERE ID='" & Me.dgProductType.CurrentRow.Cells(0).Tag & "'", ConnectionDB)
                Call open_connection()
                Dim save As Integer
                save = cmd.ExecuteNonQuery()
                If save = 1 Then
                    cmd = New MySqlCommand("UPDATE product_model SET " _
                                           & "p_name='" & Me.txtEdit.Text & "' WHERE p_name='" & Me.dgProductType.CurrentRow.Cells(0).Value & "'", ConnectionDB)
                    Call open_connection()
                    cmd.ExecuteNonQuery()
                    MessageBox.Show(Me, "บันทึกข้อมูลแล้ว", "แจ้งเตือน", MessageBoxButtons.OK, MessageBoxIcon.Information)
                    Call startfrm()
    ...
    Private Sub DeleteToolStripButton_Click(sender As Object, e As EventArgs) Handles DeleteToolStripButton.Click
        If MessageBox.Show(Me, "การลบยี่ห้อ รุ่นสินค้าจะถูกลบไปด้วยทั้งหมด ต้องการลบหรือไม่", "ยืนยัน", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = Windows.Forms.DialogResult.Yes Then
            Call DeleteData()
        End If
    End Sub
    ...
    Private Sub DeleteData()
        Try
            cmd = New MySqlCommand("DELETE FROM product_name WHERE p_name='" & Me.txtEdit.Text & "'", ConnectionDB)
            Call open_connection()
            Dim save As Integer
            save = cmd.ExecuteNonQuery()
            If save = 1 Then
                cmd = New MySqlCommand("DELETE FROM product_model WHERE p_name='" & Me.txtEdit.Text & "'", ConnectionDB)
                Call open_connection()
                cmd.ExecuteNonQuery()
    ...
    Private Sub btnHelp_Click(sender As Object, e As EventArgs) Handles btnHelp.Click
        Try
            Process.Start("https://sites.google.com/site/pakyaudio/home/serviceticker/comfix/comfixconfig/productnamefrm")
        Catch ex As Exception
